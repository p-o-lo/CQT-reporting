...
def QFT(qubits_list, nshosts, via_client, client):                      
    ...
    # Run on remote QPU
    if via_client:
        job = client.run_circuit(circuit, device='sinq20', project="personal", nshots=nshosts)
...
def main(qubits_list, device, nshots, via_client):
    ...
    result = QFT(qubits_list, nshots, via_client, client)    
    ...
    output_path = os.path.join(output_dir, f"QFT_on_{qubits_list}_{nshots}shots_device_{device}.json")
